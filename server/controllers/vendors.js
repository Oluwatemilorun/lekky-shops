...
// let { categories } = require('./default'); // TODO: change this to category model
...
exports.loadVendorProfile = async (req, res, next) => {
  try {
    const vendor = await Vendor.findById(
      { user: req.decoded.id },
      { hash: 0, isActive: 0, token: 0, isVerified: 0 }
    );
    ...
    res.status(200).send({
      state: 'OK',
      message: 'Vendor profile loaded successfully',
      payload: vendor
    });
  } catch (error) {
    console.log(error);
